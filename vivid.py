...
def login():
    ...
    if request.args.get('user') is not None:
        if request.args.get('user') != app.config['USERNAME']:
    ...
    if request.args.get('password') is not None:
        if request.args.get('password') != app.config['PASSWORD']:
            ...
            ret=dict(valid=True)
            return jsonify(ret)  
    if request.method=='POST' and request.form.get('user','') == app.config['USERNAME'] and request.form.get('password','') == app.config['PASSWORD']:
